...
def main():
    ...
    for line in input.split('\n'):
      ...
      if level in range(level_prev-3-1, level_prev): # e.g. "7,6" then 6 is in range(7-4, 7) -> yes so decreasing
        decreasing = True
      elif level in range(level_prev+1, level_prev+3+1):  # e.g. "6,7" then 7 is in range(6+1, 6+4) -> yes so increasing
        increasing = True
      else:
         continue
      ...
      if decreasing:
        for i in range(2, len(level_list)):
          ...
          if level_list[i] not in range(level_prev-3-1, level_prev):
            if args.debug:
             print("unsafe")
            unsafe = True
            break
